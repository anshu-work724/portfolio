...
const experience: Experience[] = [
  {
    ...
    timeline: "Aug 2025 - Sep 2025",
    summary:
      "Handled frontend and backend responsibilities for a scalable realtime messaging product.",
    highlights: [
...
export default function App() {
  ...
  return (
    ...
    >
      ...
      />
      <motion.div
        className="fixed left-0 top-0 z-50 h-1 origin-left bg-[#f7f2e8]"
      ...
      <section
        id="top"
        className="relative z-20 min-h-screen px-5 pt-28 sm:px-8 lg:px-10"
      >
        <motion.div
          style={{ y: heroY }}
          className="mx-auto grid min-h-[calc(100vh-7rem)] max-w-7xl items-center gap-12 lg:grid-cols-[1.05fr_0.95fr]"
        >
          <motion.div
            initial="hidden"
            animate="visible"
          ...
          >
            <motion.div
              variants={reveal}
              className="mb-9 flex flex-wrap items-center gap-x-5 gap-y-3 text-xs uppercase tracking-[0.34em] text-cyan-100/80"
            >
              <span className="flex items-center gap-3">
                <Sparkles className="h-4 w-4" /> MERN Developer
              </span>
            ...
            <motion.h1
              variants={reveal}
              className="text-[18vw] font-black uppercase leading-[0.72] tracking-[-0.1em] text-[#f7f2e8] sm:text-[8.8rem] lg:text-[10.4rem]"
            >
              Ansh
              <span className="block pl-[0.12em] text-transparent [-webkit-text-stroke:1.5px_#f7f2e8]">
                Tyagi
              </span>
            </motion.h1>
            <motion.p
              variants={reveal}
              className="mt-8 max-w-2xl text-xl leading-8 text-zinc-300 sm:text-2xl"
            >
              {profile.objective}
            </motion.p>
            <motion.div
      ...
      <section id="work" className="relative z-20 px-5 py-24 sm:px-8 lg:px-10">
        <div className="mx-auto max-w-7xl">
          <SectionHeader
            ...
            title="Three product stories from the resume, redesigned as interactive case studies."
          />
      ...
      <section
        id="experience"
        className="relative z-20 px-5 py-24 sm:px-8 lg:px-10"
      >
        <div className="mx-auto grid max-w-7xl gap-14 lg:grid-cols-[0.75fr_1.25fr]">
          <SectionHeader
            kicker="Experience"
            title="Practical internship work across the stack."
          />
                ...
                <article className="relative border-l border-cyan-200/40 pl-8">
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-5">
                    <div>
                      ...
                      <h3 className="mt-3 text-4xl font-black tracking-[-0.06em] text-[#f7f2e8]">
                        {item.role}
                      </h3>
                    </div>
                  ...
                  <ul className="mt-7 grid gap-4 md:grid-cols-3">
                    {item.highlights.map((highlight) => (
                      <li
                        key={highlight}
                        className="border-t border-white/10 pt-4 text-zinc-400"
                      >
                        {highlight}
                      </li>
                    ))}
      ...
      >
        <div className="mx-auto max-w-7xl">
          <SectionHeader
            kicker="Skills"
            title="A practical toolkit for shipping MERN applications end to end."
          />
                  ...
                  <div className="mt-6 space-y-3">
                    {items.map((item) => (
                      <motion.p
                        key={item}
                        whileHover={{ x: 8 }}
                        className="text-lg text-zinc-300 transition group-hover:text-zinc-200"
                      >
                        {item}
                      </motion.p>
                    ))}
      ...
      <section className="relative z-20 px-5 py-24 sm:px-8 lg:px-10">
        <div className="mx-auto grid max-w-7xl gap-14 lg:grid-cols-[0.75fr_1.25fr]">
          <SectionHeader
            kicker="Education"
            title="CS fundamentals with workshop and hackathon momentum."
          />
          <div>
            <Reveal>
              <div className="divide-y divide-white/10 border-y border-white/10">
                {profile.education.map((item) => (
                  <div
                    ...
                    className="grid gap-3 py-7 md:grid-cols-[1fr_auto]"
                  >
                    <div>
                      <h3 className="text-3xl font-bold tracking-[-0.05em] text-[#f7f2e8]">
                        {item.institution}
                      </h3>
      ...
      <section
        id="contact"
        className="relative z-20 px-5 pb-12 pt-24 sm:px-8 lg:px-10"
      >
        <Reveal>
  ...
  return (
    <header className="fixed left-0 right-0 top-0 z-40 px-5 py-5 sm:px-8 lg:px-10">
      <nav className="mx-auto flex max-w-7xl items-center justify-between border-b border-white/10 pb-4 backdrop-blur-md">
        <a
          href="#top"
          className="text-sm font-black uppercase tracking-[0.45em] text-[#f7f2e8]"
        >
          AT
        </a>
        <div className="hidden items-center gap-8 text-sm text-zinc-300 md:flex">
          {navItems.map((item) => (
            <a
              ...
              className="relative transition hover:text-cyan-100"
            >
              {item}
            </a>
          ))}
        ...
        <a
          href={`mailto:${profile.email}`}
          className="hidden items-center gap-2 text-sm font-semibold text-cyan-100 sm:flex"
        >
        ...
        <div className="flex h-full flex-col justify-between bg-[#05060a] p-5">
          ...
          <div>
            ...
            <p className="mt-2 text-sm text-zinc-400">
              Secure APIs, realtime events, MongoDB models.
            </p>
          </div>
        </div>
      </motion.div>
...
function ProjectPreview({ project }: { project: Project }) {
  return (
    <Reveal>
      <motion.div
        layout
        className="sticky top-28 overflow-hidden border border-white/10 bg-white/[0.03] p-5 backdrop-blur-xl"
      >
        <motion.div
          key={project.name}
      ...
      <div className="grid gap-7 lg:grid-cols-[auto_1fr]">
        ...
        <div>
          <div className="flex flex-wrap items-end justify-between gap-4">
            <h3
              className={`text-4xl font-black tracking-[-0.06em] transition sm:text-5xl ${active ? "text-cyan-100" : "text-[#f7f2e8]"}`}
            >
              {project.name}
            </h3>
          ...
          >
            <ul className="mt-6 grid gap-3 text-zinc-400 md:grid-cols-3">
              {project.outcomes.map((outcome) => (
                <li key={outcome} className="border-l border-white/10 pl-4">
                  {outcome}
                </li>
              ))}
            </ul>
  ...
  return (
    <Reveal>
      <div>
        ...
        <h2 className="mt-5 max-w-5xl text-4xl font-black leading-[0.95] tracking-[-0.065em] text-[#f7f2e8] sm:text-6xl lg:text-7xl">
          {title}
        </h2>
      </div>
    </Reveal>
...
function SectionKicker({ children }: { children: ReactNode }) {
  return (
    <p className="text-sm uppercase tracking-[0.45em] text-cyan-100/70">
      {children}
    </p>
  );
}
  ...
  return (
    <motion.a
      ...
      className={`group inline-flex items-center gap-3 px-6 py-4 font-bold transition ${
        primary
          ? "bg-[#f7f2e8] text-zinc-950 hover:bg-cyan-100"
          : "border border-white/15 text-[#f7f2e8] hover:border-cyan-100 hover:text-cyan-100"
      }`}
    >
      {label}
  ...
  return (
    <div className="mt-7 flex flex-wrap gap-2">
      {items.map((item) => (
        <span
          key={item}
          className="border border-white/10 bg-white/[0.03] px-3 py-1.5 text-xs uppercase tracking-[0.2em] text-zinc-300"
        >
          {item}
        </span>
      ))}
...
function ContactLink({
  ...
  return (
    <a
      ...
      rel="noreferrer"
      className="group flex items-center gap-3 text-zinc-300 transition hover:text-cyan-100"
    >
      <Icon className="h-5 w-5 text-cyan-200" />
      <span>{label}</span>
